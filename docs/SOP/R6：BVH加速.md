...
### 階段 1 撤回摘要（完整見 R6-1 SOP §R6-1-W）

```
ralplan 預測 vs 實機結果
  ralplan v3 預測：noise σ 抑制 ≥30%（量化指標 I），但視覺感受可能微弱
  實機驗證：使用者主觀「效果太差，只變糊不變乾淨」→ 視覺 fail
  共識退場條件 R3 觸發：階段 1 不算成功，跳階段 2 而非調強 sigma

物理上限教訓（4 條，詳見 R6-1 SOP §R6-1-W）
  1. Cross-bilateral 5×5 不是 denoise，是 spatial blur with edge preservation
  2. erichlof pixelSharpness 不是 edge mask（ralplan v3 修訂 #4 設計錯誤）
  3. ralplan 三輪共識不等於物理現實保證
  4. 視覺效果類功能應加「使用者主觀 PASS/FAIL」門檻取代純量化指標
```
