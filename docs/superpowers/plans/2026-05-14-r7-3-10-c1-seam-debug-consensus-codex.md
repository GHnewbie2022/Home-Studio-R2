...
日期：2026-05-14 起草；2026-05-15 更新至 Phase 2 第一刀執行結果
狀態：Phase 1 根因調查完成；Phase 2 第一刀 H8 + C' 已實作並通過自動驗證，同視角實機驗收待使用者確認。
...
Phase 2 第一刀已完成。後續依使用者同視角驗收結果，決定是否進 B' probe 與 H7 guard；不回到舊 flood-fill / contact invalid 路線。

## 給 OPUS 的目前讀取狀態

本檔已更新到 CODEX 完成 Phase 1 A / B 靜態 code 檢查、C / D / E raw atlas + metadata 讀檔量化，以及使用者補充確認 H7 相機位於地板實體內部與 Step F 多視角觀察後的最新進度。Phase 2 第一刀執行檔已另立於 `2026-05-14-r7-3-10-c1-phase-2-design-codex.md`。
...
| - [x] | B / H7 short-circuit 範圍檢查 | 靜態 code 檢查已完成。floor short-circuit 目前缺少 inside-geometry / ray-side guard；BoxIntersect 的內部出射資訊未被 R7-3.10 short-circuit 使用。使用者已補充確認相機整個進入地板實體空間內部，且能在內部移動看到多個 baked surface 貼圖發光、家具區呈黑色陰影。B' 剩 shader 當下數值 probe。 |
...
| 排序 | 假設 | 狀態 |
|---|---|---|
| 1 | H8：per-surface 開關沒有真正隔離，R7-3.10 全域狀態污染 R7-3.8 嫩芽路徑。 | 已由 code 檢查支持。 |
| 2 | H7：floor short-circuit 套用範圍太寬，包含穿模、背面或內面視角。 | 已由靜態 code 條件缺口與使用者相機位置補充支持；使用者已確認相機進入地板實體內部。B' 剩 shader 數值量化。 |
| 3 | C'：bake UV 半 texel 偏移。 | Phase 2 設計已鎖定：`gl_FragCoord.xy` 已是 fragment center，bake capture path 再加 `vec2(0.5)` 造成 metadata 與 bake 取樣相差半 texel。fixed-X / wardrobe xMin 黑線由此解釋。 |
| 4 | H5 / H3'：atlas 已含家具 footprint 暗區與 shared occluder / alpha policy 缺口。 | raw atlas 已量化確認暗帶；衣櫃相關範圍 atlas alpha 全為 1.0。第一輪先不做 alpha mask，驗收後再決定是否啟動第二輪。 |
| 5 | H6 / H1b：runtime hit / UV / epsilon 或 NearestFilter texel-center 選取。 | H1b 泛化 U 軸版本正式撤回；C' 已用 bake UV 半 texel 偏移收斂 fixed-X 特例；H6 只保留為 B' / H7 probe 後的 residual 檢查。 |
| 6 | H4：透視壓縮。 | 已由 Step F 排除。 |
...
| - [ ] | 實機驗收 | 等使用者在同視角確認 floor / north 西側黑線是否退掉，以及 north 開啟時嫩芽是否仍存在。 |
| - [ ] | B' / H7 | 待第一刀實機結果。若仍有內部發光或暗區貼回，再進 probe 與 guard。 |
...
| - [x] | H7 | floor short-circuit 套用範圍太寬，包含穿模、背面或內面視角。 | 第二 | 靜態 code 條件缺口成立：目前 short-circuit 缺少 inside-geometry / ray-side guard。使用者已確認相機整個進入地板實體空間內部；B' 剩 shader 當下數值 probe。 |
...
| - [x] | 2 | H7 | 靜態 code 已確認：floor short-circuit 只看 object / normal / position，沒有 inside-geometry / ray-side guard；BoxIntersect 內部出射資訊未傳到 short-circuit。 |
| - [x] | 3 | H5 / H3' | raw atlas 已確認暗帶存在；fixed-X 邊界外側第一格直接變暗，fixed-Y / fixed-Z 接縫第一格維持亮值；衣櫃相關範圍 atlas alpha 全為 1.0。 |
| - [x] | 4 | H6 / H1b | east wall U 軸邊界沒有 fixed-X 類外側黑線，H1b 泛化 U 軸版本正式撤回；H6 留待 runtime 動態 probe。 |
| - [x] | 5 | H4 | Step F 已正式排除。 |
...
| - [x] | B / H7 | `r7310C1FullRoomDiffuseShortCircuit()` 在 floor mode 開啟時，只要 floor surface 判定與 UV 判定通過，就讀 atlas 並 return true。 | baked radiance 可被直接加進 `accumCol`，符合「相機在地板實體內部仍看見 baked surface 發光」的使用者補充；B' 剩 shader 數值量化。 | `shaders/Home_Studio_Fragment.glsl:543-555` |
...
Phase 2 第一刀已完成，詳見 `2026-05-14-r7-3-10-c1-phase-2-design-codex.md`。
下一步由使用者做同視角實機驗收：floor 西側黑線、north 西側黑線、north 開啟時嫩芽共存。若第一刀仍留下內部發光或暗區貼回，後續順序為 B' probe、H7 guard、第二階段實機驗收。
H5 / H3' alpha mask 與 East wall runtime 保留為第二輪候選。
