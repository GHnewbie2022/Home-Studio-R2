...
assert.equal(accepted.routeStatus, 'invalidated_by_ab_visual_check');
...
assert.equal(accepted.visualAcceptance.acceptedByUser, false);
assert.equal(accepted.visualAcceptance.invalidatedByUser, true);
...
assert.equal(accepted.validation.automatedStatus, 'ab_visual_check_invalidated_previous_acceptance');
assert.equal(accepted.validation.userVisualAcceptance, 'fail');
assert.equal(accepted.validation.latestActualSamples, 1000);
assert.equal(accepted.validation.cameraStateVariation, true);
assert.equal(accepted.validation.abVisualCheck.samples, 1);
...
assert.match(html, /id="r739-sprout-ab-actions"/);
assert.match(html, /id="btn-r739-ab-a"/);
assert.match(html, /id="btn-r739-ab-b"/);
assert.match(html, /id="btn-r739-ab-c"/);
assert.match(html, /id="btn-r739-ab-d"/);
assert.match(homeStudio, /bindR739SproutABControls/);
assert.match(homeStudio, /refreshR739SproutABButtons/);
...
assert.match(initCommon, /let r739C1CurrentViewReflectionPreviewEnabled = false;/);
assert.match(initCommon, /let r739SproutABMode = 'A';/);
...
assert.match(initCommon, /window\.setR739SproutABMode/);
assert.match(initCommon, /window\.reportR739SproutABMode/);
assert.match(initCommon, /window\.logR739SproutABMode/);
assert.match(initCommon, /diffuseBakeApplied/);
assert.match(initCommon, /currentViewReflectionApplied/);
assert.match(initCommon, /diffuseWouldBeBlockedByCurrentView/);
