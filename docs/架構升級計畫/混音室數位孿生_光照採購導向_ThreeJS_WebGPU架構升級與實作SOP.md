...
## 2026-05-16 現況覆寫：本文件改列為候選架構評估

```
目前主線
  先留在現有 Home Studio 架構。
  把快速預覽模式收成 hybrid room：
    靜態漫射面讀 bake。
...
目前不採本文件原先的立即搬遷建議
  WebGPU / SSGI / LightProbeGrid 對互動很有價值，但會換掉大量 shader / 材質 / 後處理路線。
  R7-3.10 現在仍在解 bake / LIVE 交界與覆蓋面問題。
  在光學答案未收斂前搬架構，會同時引入 bake 問題、匯出問題、材質重建問題與新 renderer 問題。

快速預覽成功後的下一層議題
  開「高品質 bake 生產線」專案。
  先量化趨近真實模式的 bake 成本：
    單面 1000 / 20000 SPP 花多久。
    全相關靜態漫射面需要多少 surface。
    哪些面真的需要 14 bounces。
    哪些資料可分燈、分面、分批、快取、續跑。
...
候選架構定位
  WebGPU：
    網頁端互動與運算管線重建候選。
    適合等高品質 bake 成本被量化後，再決定是否投入。
  Apple Metal：
    本機高品質 bake 工廠候選。
    適合 M 系列硬體加速光追，但會離開目前網頁主線。
  Blender / Cycles / OIDN：
    採購級外部驗證與高品質離線輸出候選。
    需要建立幾何、材質、燈光與 IES 對齊流程。
  PlayCanvas：
    展示承載候選。
    等 baked room 在 Home Studio 內驗證穩定後，再用來做可走動、可部署、操作舒服的版本。

讀法
  本文以下章節保留技術評估價值。
  「第一階段直接 WebGPU 升級」相關文字視為歷史方案，不作為目前下一刀。
```
